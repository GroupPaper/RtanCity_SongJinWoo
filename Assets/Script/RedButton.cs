...
public class RedButton : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other) // is trigger가 체크된 콜라이더에 범위안으로
    {
        if (other.CompareTag("Player")) // 오브젝트 태그가 Player면 활성
        {
    ...
    private void OnTriggerExit2D(Collider2D other) // is trigger가 체크된 콜라이더에 범위밖으로
    {
        if (other.CompareTag("Player")) // 오브젝트 태그 Player가 벗어나면 비활성
        {
